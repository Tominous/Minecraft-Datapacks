...
execute as @a unless score @s a.healthold matches 0 if score @s a.health > @s a.healthold run tellraw @a [{"selector":"@s","color":"green"},{"text":" restored "},{"score":{"name":"@s","objective":"a.healtloss"}},{"text":" health ["},{"score":{"name":"@s","objective":"a.health"}},{"text":"HP]"}]
